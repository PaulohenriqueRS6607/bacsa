...
export default function BooksScreen() {
  ...
  const fetchAllCategoriesBooks = async () => {
    try {
      ...
      // Buscar livros para categorias iniciais (limitamos a 3 para não sobrecarregar)
      const categoryIds = CATEGORIES.slice(0, 3).map(cat => cat.id);
      const categoriesData = await booksApiService.getAllCategoriesBooks(categoryIds);
